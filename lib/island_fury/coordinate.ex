defmodule IslandFury.Coordinate do
  ...
  def guess(coordinate) do
    Agent.update(coordinate, fn state -> %Coordinate{state | guessed?: true} end)
  end

  def set_in_island(coordinate, value) when is_atom value do
    Agent.update(coordinate, fn state -> %Coordinate{state | in_island: value} end)
  end
  ...
  def get_state(coordinate) do
    Agent.get(coordinate, fn state -> state end)
  end
end